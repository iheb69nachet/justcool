...
function ProductCard({ product, onClick }: { product: Product; onClick?: (p: Product) => void }) {
  const { name, mainIngredient, extras, price, menuUpcharge = 2, studentDiscount = 1, imageSrc, isNew } = product;
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative w-full aspect-[4/3] shrink-0 overflow-hidden rounded-t-xl bg-gray-900" aria-hidden="true">
        <img
          alt=""
          loading="lazy"
        ...
        {isNew && (
          <div className="absolute top-3 right-3">
            <span className="inline-flex items-center justify-center rounded-sm px-2 py-0.5 text-xs
      ...
      <div className="flex flex-col flex-grow px-4 pt-4 pb-6 w-full text-left">
        ...
        <div className="mt-auto" aria-hidden="true">
          <div className="font-black text-red-600 text-3xl mb-2">{price.toFixed(2)}€</div>
          ...
          <p className="text-white/40 text-xs whitespace-nowrap mb-2">
            <span className="font-bold">-{studentDiscount}€</span> pour étudiants
          </p>
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ contentVisibility: "auto", containIntrinsicSize: "350px" } as React.CSSProperties}
      >
        {/* Section heading */}
        <h2
          id={categoryId}
        ...
        >
          <span
            aria-hidden="true"
            className="uppercase text-[2.25rem] font-bold tracking-wider text-white"
          ...
          <span
            aria-hidden="true"
            className="uppercase text-[2.25rem] font-bold tracking-wider text-red-600"
            style={{ textShadow: "rgba(220,38,38,0.5) 0px 0px 20px" }}
          >
            {categoryLabel}
          </span>
